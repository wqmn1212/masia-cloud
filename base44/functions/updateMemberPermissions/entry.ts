...
Deno.serve(async (req) => {
  try {
    ...
    const { target_user_id, allowed_tabs } = await req.json();
    if (!target_user_id || !Array.isArray(allowed_tabs)) {
    ...
    await base44.asServiceRole.entities.User.update(target.id, { allowed_tabs });
    return Response.json({ ok: true, allowed_tabs });
  } catch (error) {
