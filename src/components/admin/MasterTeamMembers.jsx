...
export default function MasterTeamMembers({ tenantId }) {
  ...
  return (
    <Card>
      <CardHeader className="flex-row items-center justify-between"><CardTitle className="flex items-center gap-2 text-base"><Users className="h-4 w-4" />{isClientTeam ? '고객 계정' : '팀원 관리'} ({members.length})</CardTitle>{!isClientTeam && <MasterTeamInviteDialog tenantId={tenantId} />}</CardHeader>
      <CardContent className="divide-y p-0">
        {isLoading ? <Loader2 className="mx-auto my-8 h-5 w-5 animate-spin" /> : members.length === 0
          ...
          : members.map((member) => <UserAccountRow key={member.id} user={member} disabled={toggle.isPending} onToggle={(is_active) => toggle.mutate({ id: member.id, is_active })} onResetPassword={isClientTeam ? () => resetPassword.mutate(member.email) : undefined} resetPending={resetPassword.isPending} />)}
        {pending.map((invite) => <div key={invite.id} className="flex items-center gap-3 p-3"><Mail className="h-4 w-4 text-muted-foreground" /><div className="flex-1"><p className="text-sm font-medium">{invite.email}</p><p className="text-xs text-muted-foreground">{isClientTeam ? '초대 메일에서 가입 후 비밀번호를 설정합니다' : (invite.team_role_name || '팀원')}</p></div><span className="text-xs font-medium text-amber-600">가입 대기 중</span></div>)}
      </CardContent>
    </Card>
  );
}