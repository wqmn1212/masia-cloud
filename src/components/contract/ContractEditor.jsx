...
export default function ContractEditor({ form, onChange, quotations, onApplyQuotation, onSave, onReset, isPending, bodyRef }) {
  return (
    <Card>
      <CardContent className="p-4 space-y-4">
            ...
            <SelectContent>
              {quotations.map(q => (
                <SelectItem key={q.id} value={q.id}>
                  {(q.quote_title || q.product_name || '무제')} · {q.client_name || '-'}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <Button className="w-full" onClick={onSave} disabled={isPending || !form.contract_title}>
