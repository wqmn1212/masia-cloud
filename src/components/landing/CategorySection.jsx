    ...
    <section id="category" className="border-t border-landing-line bg-white px-5 lg:px-8 py-16 lg:py-[88px]">
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end gap-4 lg:gap-8 mt-3.5">
          <SectionHeading eyebrow={null} title={tx(categories.h2, lang)} maxWidth="max-w-[560px] !mt-0" />
        ...
        <div className="flex flex-wrap gap-2 mt-8">
          {categories.tabs.map((t) => (
            <button
              ...
              onClick={() => setCat(t.id)}
              className={cn(
                'flex-none whitespace-nowrap border text-[13.5px] font-bold px-[15px] py-[9px] rounded-full transition-colors',
                cat === t.id ? 'bg-landing-brand border-landing-brand text-white' : 'bg-white border-landing-line2 text-landing-ink2 hover:border-landing-brand'
              )}
            >
              {tx(t.label, lang)}
            </button>
